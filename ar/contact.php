...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($nom) || empty($email) || empty($sujet) || empty($message)) {
        ...
        $email_body = "لقد تلقيت رسالة جديدة من محفظتك.\n\n";
        $email_body .= "الاسم: $nom\n";
        $email_body .= "البريد الإلكتروني: $email\n";
        $email_body .= "الموضوع: $sujet\n\n";
        $email_body .= "الرسالة:\n$message\n";
...
$pageTitle = 'اتصل - Flavien GARIBALDI';
$currentPage = 'contact.php';
include '../includes/header.php';
?>

    <section id="contact" style="margin-top: 100px;">
        <h2 class="fade-up">اتصل بي</h2>
        <p style="color: var(--text-gray); font-size: 1.1rem; margin-bottom: 3rem; text-align: center;">
            هل لديك سؤال أو مشروع أو ترغب فقط في التواصل؟<br>
            لا تتردد في الاتصال بي من خلال النموذج أدناه.
        </p>
            ...
            <form method="POST" action="contact.php">
                <div class="form-row">
                    <div class="form-group">
                        <label for="nom"><i class="fas fa-user"></i> الاسم الكامل *</label>
                        <input type="text" id="nom" name="nom" required placeholder="اسمك" value="<?php echo isset($nom) ? $nom : ''; ?>">
                    </div>
                ...
                <div class="form-group">
                    <label for="sujet"><i class="fas fa-tag"></i> الموضوع *</label>
                    <input type="text" id="sujet" name="sujet" required placeholder="موضوع رسالتك" value="<?php echo isset($sujet) ? $sujet : ''; ?>">
                </div>

                <div class="form-group">
                    <label for="message"><i class="fas fa-comment"></i> الرسالة *</label>
                    <textarea id="message" name="message" required placeholder="رسالتك..."><?php echo isset($message) && !$message_sent ? $message : ''; ?></textarea>
                </div>
        ...
        <div style="margin-top: 4rem; text-align: center;">
            <h3 style="margin-bottom: 1.5rem; color: var(--text-light);">تابعني أيضًا على</h3>
