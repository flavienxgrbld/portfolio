...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($nom) || empty($email) || empty($sujet) || empty($message)) {
        ...
        $error_message = "O endereço de email não é válido.";
    } else {
        $to = "flavien.garibaldi@gmail.com";
        $email_subject = "Portfolio Contact: " . $sujet;
        $email_body = "Recebeu uma nova mensagem do seu portfolio.\n\n";
        $email_body .= "Nome: $nom\n";
...
?>
    ...
    <section id="contact" style="margin-top: 100px;">
        <h2 class="fade-up">Contactar-me</h2>
        <p style="color: var(--text-gray); font-size: 1.1rem; margin-bottom: 3rem; text-align: center;">
            Tem uma questão, um projeto ou simplesmente deseja conversar?<br>
            Não hesite em contactar-me através do formulário abaixo.
        </p>
            ...
            <form method="POST" action="contato.php">
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" id="nom" name="nom" required placeholder="O seu nome" value="<?php echo isset($nom) ? $nom : ''; ?>">
                    </div>

                    <div class="form-group">
                        <label for="email"><i class="fas fa-envelope"></i> Email *</label>
                        <input type="email" id="email" name="email" required placeholder="o.seu.email@exemplo.pt" value="<?php echo isset($email) ? $email : ''; ?>">
                    </div>
                </div>

                <div class="form-group">
                    <label for="sujet"><i class="fas fa-tag"></i> Assunto *</label>
                    <input type="text" id="sujet" name="sujet" required placeholder="O objeto da sua mensagem" value="<?php echo isset($sujet) ? $sujet : ''; ?>">
                </div>

                <div class="form-group">
                    <label for="message"><i class="fas fa-comment"></i> Mensagem *</label>
                    <textarea id="message" name="message" required placeholder="A sua mensagem..."><?php echo isset($message) && !$message_sent ? $message : ''; ?></textarea>
                </div>
        ...
        <div style="margin-top: 4rem; text-align: center;">
            <h3 style="margin-bottom: 1.5rem; color: var(--text-light);">Encontre-me também em</h3>
