...
;!function(win){
  ...
  ,modules = {
    ...
    ,util: 'modules/util' // 
    ,code: 'modules/code' // 
    ,jquery: 'modules/jquery' //DOM （ ）
    
    ,mobile: 'modules/mobile' //  |  ， ， 
    ,'layui.all': '../layui.all' //PC 
  };
    ...
    ,result = {
      ...
      ,ie: function(){ //ie 
        return (!!win.ActiveXObject || "ActiveXObject" in win) ? (
          (agent.match(/msie\s(\d+)/) || [])[1] || '11' // ie11 msie 
        ) : false;
      }()
  ...
  Layui.prototype.event = Layui.event = function(modName, events, params, fn){
    var that = this
    ,result = null
    ,filter = events.match(/\((.*)\)$/)||[] // ， ：select(xxx)
    ,eventName = (modName + '.'+ events).replace(filter[0], '') // ， ：form.select
    ,filterName = filter[1] || '' // ,， ：xxx
